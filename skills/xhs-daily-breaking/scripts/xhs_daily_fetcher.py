...
def fetch_explosive_articles(rank_date: str, category: str = "综合全部") -> dict:
    """
    调用API获取小红书爆款内容数据（使用原生socket+ssl，不发送SNI）
...
"""
    ...
    try:
        ...
        # 包装socket时不传server_hostname（不发送SNI）
        ...
        if '200' not in first_line:
            raise Exception(f"HTTP请求失败: {first_line}")
...
def format_ranking_list(articles: list, category: str = "综合全部") -> str:
    ...
    output_lines.append(f"**🔥 【{clean_text(category)}】单日爆发榜**")
    output_lines.append("*单日爆发榜指笔记发后第3天比第2天涨的数据由高到低Top50*")
