...
def fetch_xhs_weekly(rank_date=None, category="综合全部"):
    ...
    path = f"/story/cozeSkill/getXhsCozeSkillDataOne?rankDate={rank_date}&source={source}&category={category_encoded}"
...
"""
    ...
    if '\r\n\r\n' not in response_str:
        return {"fetch_time": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "query_type": "日榜", "category": category, "hot_list": []}
...
def generate_html(result, top_n=20):
    ...
    page_title = f"小红书单日爆发榜 - {category}"
...
<div class="page-wrap">
    ...
    <div class="pdf-content" id="pdfContent">
        <div class="header-wrap">
            <h1>📱 小红书单日爆发榜<span class="category-badge">{category}</span></h1>
            <div class="header-meta">更新时间：{fetch_time}</div>
            <div style="font-size:12px;color:rgba(255,255,255,0.75);margin-top:4px;">*单日爆发榜指笔记发后第3天比第2天涨的数据由高到低Top50*</div>
            <div class="stats-row">
                <div class="stat-item">
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='生成小红书单日爆发榜HTML页面')
