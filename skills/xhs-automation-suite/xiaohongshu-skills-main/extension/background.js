...
async function handleCommand(msg) {
  ...
  switch (method) {
    ...
    // 通用导航：打开任意 URL（不限于 xiaohongshu.com）
    case "navigate_any":
      return await cmdNavigateAny(params);
...
async function cmdWaitForLoad({ timeout = 60000 }) {
  const tab = await getOrOpenAnyTab();
  await waitForTabComplete(tab.id, null, timeout);
  return null;
}

// 通用导航：打开新标签加载任意 URL。供非 XHS 站点（如 ziyuan.baidu.com）使用。
let __lastNavigatedTabId = null;
async function cmdNavigateAny({ url, reuseTab = false }) {
  let tab;
  if (reuseTab && __lastNavigatedTabId) {
    try {
      tab = await chrome.tabs.get(__lastNavigatedTabId);
  ...
  __lastNavigatedTabId = tab.id;
  await waitForTabComplete(tab.id, null, 60000);
  return { tabId: tab.id, url: tab.url };
}

// 通用：获取最后一次 navigate_any 创建的 tab，没有则 fallback 到 XHS tab
async function getOrOpenAnyTab() {
  if (__lastNavigatedTabId) {
    try {
      const t = await chrome.tabs.get(__lastNavigatedTabId);
      if (t) return t;
    } catch {}
  }
  return await getOrOpenXhsTab();
}
