...
> ⚠️ **2026-08-24 流程变更：默认只输出水印版**（Yoyo 要求）—— 不再生成两份（客户版+水印版），直接产出水印版作为给客户的唯一交付物。

| 文件 | 用途 |
|------|------|
| `xxx_水印版.pdf` | **唯一交付**：带"品牌名 日期"水印的最终 PDF |
| `xxx.md` | 可编辑源文档 |

**实现方式：** add_watermark(临时基础版, 水印版, text=...) + os.remove(临时基础版)，中间无水印版不落地。
**文件命名：** `xxx_水印版.pdf`（去掉"客户版_"前缀，水印版即客户最终版）。
