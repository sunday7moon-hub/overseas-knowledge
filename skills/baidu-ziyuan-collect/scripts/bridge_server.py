"""Browser Bridge Server - 通用 WebSocket Bridge

Extension 连接到这里（WebSocket），CLI 命令通过同一端口发送，
Bridge 将命令路由给 Extension 并把结果返回给 CLI。

启动方式：
    python bridge_server.py --port 9333
...
class BridgeServer:
    ...
    async def _handle_cli(self, ws: ServerConnection, msg: dict) -> None:
        ...
        if not self._extension_ws:
            await ws.send(json.dumps({"error": "Extension 未连接，请确认浏览器已安装并启用 Browser Bridge 扩展"}))
            return
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Browser Bridge Server")
