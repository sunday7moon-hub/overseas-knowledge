...
npx -y socialdatax-skills@latest xhs search \
  ...
  --source-platform skillhub --source-skill xhs-topic-analysis-v2
...
- 可选：`--source-client socialdatax-skills --source-platform skillhub --source-skill xhs-topic-analysis-v2`：这是当前 Agent Skill 的来源标记；按本 Skill 示例执行时保持这些值不变。
